...
int main(int argc, char *argv[])
{
    ...
    if (argInput)
        inputFileName = args::get(argInput); // input file is mandatory positional argument. Overrides any definition in configuration.yaml
    if (argOutput)
        outputFileName = args::get(argOutput); // input file is mandatory positional argument. Overrides any definition in configuration.yaml
    if (argVerbose)
        params.verbosity = args::get(argVerbose); // retrieve user defined verbosity level
    ...
    if (argAlphaRadius)
        params.alphaShapeRadius = args::get(argAlphaRadius);
    if (argGroundThreshold)
        params.groundThreshold = args::get(argGroundThreshold);
    if (argHeightThreshold)
        params.heightThreshold = args::get(argHeightThreshold);
    if (argSlopeThreshold)
        params.slopeThreshold = args::get(argSlopeThreshold);
    if (argRobotHeight)
        params.robotHeight = args::get(argRobotHeight);
    if (argRobotLength)
        params.robotLength = args::get(argRobotLength);
    if (argRobotWidth)
        params.robotWidth = args::get(argRobotWidth);
    if (argProtrusionSize)
        params.protrusionSize = args::get(argProtrusionSize);
    if (argRotation)
    {
    ...
    if (argMetacenter)
        params.ratioMeta = args::get(argMetacenter);
    if (argSaveIntermediate)
        params.exportIntermediate = args::get(argSaveIntermediate);
    ...
    if (params.verbosity > 0)
    {
        logc.debug("main", "Joining queue for Lane C [& X]");
        tt.lap("** M1 & C1");
    }
